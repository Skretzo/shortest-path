...
public class QuetzalTransportTest {
    ...
    public void testQuetzalTransportsFromActualResource() throws IOException {
        ...
        // Check that Cam Torum destination has VarPlayer check (4182&32)
        int camTorumPacked = WorldPointUtil.packWorldPoint(1446, 3108, 0);
        boolean foundCamTorumWithCheck = false;

        for (Set<Transport> transportSet : transports.values()) {
            for (Transport t : transportSet) {
                if (t.getDestination() == camTorumPacked) {
                    for (VarRequirement vp : t.getVarPlayers()) {
                        if (vp.getId() == VARPLAYER_QUETZAL_PLATFORMS && vp.getValue() == BIT_CAM_TORUM) {
                            foundCamTorumWithCheck = true;
                        }
                    }
                }
            }
        ...
        Assert.assertTrue("Cam Torum transport should have VarPlayer 4182&32 check", foundCamTorumWithCheck);
    }
