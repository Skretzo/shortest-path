...
/**
 * A lightweight hash map keyed by primitive {@code int} values with open
 * addressed bucket arrays.
 * <p>
 * Each top-level slot in {@link #buckets} references an array (the "bucket") of
 * entries. Buckets grow
 * individually when full, avoiding the cost of rehashing the entire map on
 * every local expansion. When the
 * aggregate {@linkplain #size entry count} reaches the configured
 * {@linkplain #capacity load threshold}, the
 * map resizes and rehashes all entries into a larger bucket array whose length
 * is always a power of two.
 * <p>
 * This implementation is intentionally minimal and tailored for the plugin's
 * pathfinding needs:
 * <ul>
 * <li>No support for removing entries.</li>
 * <li>No iteration views (keys, values, or entry set) are exposed.</li>
 * <li>Collisions within a bucket are handled by linear probing inside the
 * bucket array.</li>
 * <li>Duplicate key insertion replaces the previous value, or appends
 * collection contents when both the
 * old and new values are {@link Collection}s (best effort; falls back to
 * replacement on errors).</li>
 * </ul>
 *
...
public class PrimitiveIntHashMap<V>
{
	 ...
	 *
	 * @param initialSize initial expected number of elements; rounded up to
	 *                    maintain a power-of-two capacity.
	 * @param loadFactor  a value in the range {@code [0.0, 1.0]} determining when
	 *                    the map rehashes. Higher values
	 ...
	 * <p>
	 * If a mapping already exists and both the existing and new values implement
	 * {@link Collection}, the method
	 * attempts to append all elements of the new collection into the existing one.
	 * If the append fails (e.g., due
	 * to incompatible element types or an unsupported operation) the existing value
	...
	/**
	 * Calculates the percentage of bucket capacity used prior to first null
	 * sentinel entries across all buckets.
	 * <p>
	 * For each non-null bucket, usage counts entries until the first {@code null}
	 * slot (which indicates the end
	 * of populated entries for that bucket). The resulting percentage is:
	 * {@code usedEntrySlots / totalAllocatedSlots * 100}.
	 *
